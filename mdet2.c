...
#define STILLCAP "echo 'still' > /home/pi/pikrellcam/www/FIFO" //  command to ca                                                                                pture still
...
int retval = 0;
  ...
  do {
      if (fresh == TRUE) {
        ...
        fprintf(fp,"%s.%03d, ",tbuf,millisec);   // write to file including mill                                                                                iseconds
        if (g_level == 0) { // returned to zero; end of motion event
           ...
           fprintf(fp,"%d,     %4.1f\n", g_level, deltaSec);
           retval = system(STILLCAP);  // command to trigger local capture of st                                                                                ill image
...
/*
        ...
        strcpy(fname, "raspistill -t 1000 -rot 180 -o "); // command to record i                                                                                mage
        strcat(fname, tod);
        printf("Saving %s\n", tod);
        retval = system(fname);
        }
