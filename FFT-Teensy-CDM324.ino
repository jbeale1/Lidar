...
#define MOTION_THRESH (3)  // sum of alarm channels exceeding this value = motion detect
#define PEAK_THRESH (2)  // largest bin must reach this value
#define DECRATIO (8)    // decimation ratio (data processed / printed out)
#define MINRUN (2)    // minimum # of cycles of motion before validated (displayed)
...
unsigned int runLength;   // how many readings in a row "motion" has been detected
unsigned int stopLength;  // how many readings in a row there's been no motion
...
int i;
  ...
  SERPORT.print("# CMD324 Doppler FFT on Teensy 3.2 ver 1.24 31-AUG-2017");
  for (i=0; i<8; i++) {
    delay(1000);
    SERPORT.print(".");
  }
...
void updateLevel(float f) {
    ...
    level[14] = ((1.0-f)*level[11]) + f * (myFFT.read(0, 511) * A_GAIN);  // total sum
}
...
//    if ((sumAlarm >= MOTION_THRESH) && (maxVal >= PEAK_THRESH) && (iMax >= BIN_CUTOFF) ) {  // motion detected
    if ((sumAlarm >= MOTION_THRESH) && (maxVal >= PEAK_THRESH) ) {  // motion detected
      motion = true;
      runLength++;  // how many cycles we've continuously seen motion
      stopLength = 0;
      ...
      firstPrint = true;
      runLength = 0;
      stopLength++;   // how many cycles we haven't seen any motion
    }
}

// =========================================================================
// printData()  print one line of frequency data
...
void loop() {
  ...
  if (myFFT.available()) {
    ...
    if ((loops++)%DECRATIO == 0) {
      ...
      tail = (stopLength < 4*DECRATIO);
      mt = (motion | tail);
      
      if (mt) {  // show motion, plus next three cycles afterwards
        if (motionOld == false) {  // this is the first line of a new set
          SERPORT.println();  // blank line at start to separate events
        }
        if (( runLength > (MINRUN*DECRATIO) ) || (stopLength < 4*DECRATIO))  
        {
          printData();              
        } else
        { // motion, but not yet 2x samples
               for (i=0; i<BINS; i++) {         
                   hist[i] = alarm[i];  // back up for possible printout
               }
        } // else            
      } // if (mt)
      motionOld = mt;  // remember value from previous timestep
